    ...
    <div class="min-h-screen py-12 px-4 sm:px-6 lg:px-8">
        <div class="mx-auto max-w-3xl">
            <div class="mb-8 text-center">
                ...
                <p class="mt-3 text-base text-slate-600">Share your thoughts so we can keep improving your learning
                    experience.</p>
            </div>
            ...
            <div class="rounded-2xl bg-white p-6 shadow-lg ring-1 ring-slate-200 sm:p-10">
                ...
                <form method="POST" action="/feedback" class="space-y-6">
                    ...
                    <div>
                        ...
                        @if ($subjects->isEmpty())
                            <p class="mt-2 text-xs text-rose-600">No subjects assigned to your class yet. Please contact
                                your lecturer or admin.</p>
                        @endif
                    </div>
                    ...
                    <div>
                        <label for="mood_rating" class="block text-sm font-medium text-slate-700">Mood Check-In (Emosi
                            Pelajar)</label>
                        <p class="mt-1 text-sm text-slate-500">Bagaimana perasaan anda selepas kelas?</p>
                        <input id="mood_rating" type="range" name="mood_rating" min="1" max="5"
                            value="{{ old('mood_rating', 3) }}" required class="mt-4 w-full accent-indigo-600">
                    ...
                    <div class="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 px-4 py-3">
                        ...
                        <div>
                            <label for="is_anonymous" class="text-sm font-medium text-slate-700">Submit as
                                anonymous</label>
                            <p class="text-xs text-slate-500">We store your user ID for audit purposes, but it remains
                                hidden in admin views.</p>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <a href="{{ url('/') }}"
                            class="text-sm font-semibold text-slate-600 transition hover:text-slate-500">
                            Back to Home
                        </a>
                    </div>
                </form>
